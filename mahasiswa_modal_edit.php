					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<select name="Kode_Jurusan_Mhs" class="form-control">
								...
								$querymhsjrs = "SELECT Kode_Jurusan_Mhs, Kode_Jurusan, Nama_Jurusan FROM mahasiswa INNER JOIN jurusan ON Kode_Jurusan_Mhs=Kode_Jurusan WHERE NIM='$NIM'";
								$stmt = $konek->prepare($querymhsjrs);
								$stmt->execute();

								if ($querymhsjrs == false) {
									die("Terdapat Kesalahan : ");
								}
								while ($mhsjrs = $stmt->fetch()) {
									echo "<option value=$mhsjrs[Kode_Jurusan_Mhs] selected>$mhsjrs[Nama_Jurusan]</option>";
								}

								$queryjrs = "SELECT * FROM jurusan";
								$stmt = $konek->prepare($queryjrs);
								$stmt->execute();

								if ($queryjrs == false) {
									die("Terdapat Kesalahan : ");
								}
								while ($jrs =  $stmt->fetch()) {
